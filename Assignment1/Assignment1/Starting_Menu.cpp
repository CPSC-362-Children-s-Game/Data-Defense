...
Menu::Menu(float wid, float hei)
{
    ...
    menu[1].setPosition(sf::Vector2f(wid / 2, hei / (ITEMS + 1) * 2));
    menu[1].setCharacterSize(16);

    menu[2].setFont(font);
    menu[2].setString("Setting");
    menu[2].setFillColor(sf::Color::White);
    menu[2].setPosition(sf::Vector2f(wid / 2, hei / (ITEMS + 1) * 3));
    menu[2].setCharacterSize(16);

    menu[3].setFont(font);
    menu[3].setString("Exit");
    menu[3].setFillColor(sf::Color::White);
    menu[3].setPosition(sf::Vector2f(wid / 2, hei / (ITEMS + 1) * 4));
